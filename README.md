...
- **Day, week, month:** the Day / Week / Month buttons (D W M on a phone) pick what to show, and ‹ › step back and forward by that much (tap the date between them to come back to now). A week shows Monday to Sunday side by side, always fitting the screen, so on a phone its blocks show just the category. A month is a calendar: a small timeline per day, all on the same hours; tap a day's number to open it, or a block to edit it. The bar and the day names stay at the top while the timeline scrolls. `/timeline` with a range opens the view that fits it.
- **Console tray:** drag its divider to resize it (the size is remembered), or click the divider to fold it to one line and back.
- **On a phone:** the console starts folded to a one-line peek; tap the handle to open it. Swipe left or right to step through days, weeks or months. The legend is left out to save room. Tapping a block opens the editor as a bottom sheet; tap outside it or swipe it down to close it. The status bar shows sync as a colored dot. Nothing scrolls sideways: long report lines wrap under their last column, `/edit` wraps.
...
- **Losing every device and the recovery key** means the log can't be decrypted by anyone. If you can still sign in with your email, `/reset-encryption` on a locked device starts over: it deletes the synced log and settings (nobody can read them anyway), keeps entries typed on that device that were never uploaded, and makes a new key and recovery key. You have to type `DELETE` to confirm. A device that still has the old key notices (each key has a public fingerprint stored with it), stops uploading, and asks to be linked again; once it is, it uploads its copy of the log under the new key.
